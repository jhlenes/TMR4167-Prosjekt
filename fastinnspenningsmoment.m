function fim = fastinnspenningsmoment(punkt, elem, last, elementlengder)
    ...
    for i=1:nLast
        ...
        if last(i, 2) == 2 % hvis punktlast
            ...
            if punkt(p1, 3) ~= 0 && punkt(p2, 3) ~= 0  % fast innspent begge ender
                m1 = - F * a * (l-a)^2 / l^2;
                m2 =   F * a^2 * (l-a) / l^2;
            elseif punkt(p1, 3) ~= 0 % fast innspent ende 1
                b = l-a; % avstand fra 2
                m1 = - F * b * (l^2 - b^2) / (2*l^2);
                m2 = 0;
            elseif punkt(p2, 3) ~= 0 % fast innspent ende 2
                m1 = 0;
